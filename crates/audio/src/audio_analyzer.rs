...
impl AudioAnalyzer {
    ...
    pub fn analyze_spectrum(&self, audio_buffer: &crate::audio_buffer::AudioBuffer) -> SpectrumAnalysis {
        ...
        let crest_factor = if rms_level > 0.0 { peak_level / rms_level } else { 0.0 };
    ...
    pub fn analyze_waveform(&self, audio_buffer: &crate::audio_buffer::AudioBuffer) -> WaveformAnalysis {
        let samples = audio_buffer.clone_data();
    ...
    pub fn analyze_spectrogram(&self, audio_buffer: &crate::audio_buffer::AudioBuffer) -> SpectralAnalysis {
        let samples = audio_buffer.clone_data();
    ...
    pub fn calculate_spectral_centroid(&self, audio_buffer: &crate::audio_buffer::AudioBuffer) -> f32 {
        let spectrum = self.analyze_spectrum(audio_buffer);
        
        let weighted_sum: f32 = spectrum.frequencies
            .iter()
    ...
    pub fn calculate_spectral_rolloff(&self, audio_buffer: &crate::audio_buffer::AudioBuffer, rolloff_percent: f32) -> f32 {
        let spectrum = self.analyze_spectrum(audio_buffer);
    ...
    pub fn calculate_zero_crossing_rate(&self, audio_buffer: &crate::audio_buffer::AudioBuffer) -> f32 {
        let samples = audio_buffer.clone_data();
        let zero_crossings = self.count_zero_crossings(&samples);
    ...
    fn calculate_dynamic_range(&self, samples: &[f32]) -> f32 {
        ...
        let min_sample = samples.iter().fold(f32::INFINITY, |acc, &sample| acc.min(sample));
        let max_sample = samples.iter().fold(f32::NEG_INFINITY, |acc, &sample| acc.max(sample));
    ...
    pub fn set_overlap_ratio(&mut self, overlap_ratio: f32) {
        ...
        self.time_resolution = self.fft_size as f32 * (1.0 - self.overlap_ratio) / self.sample_rate as f32;
    }
