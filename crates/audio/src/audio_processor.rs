...
impl AudioProcessor {
    ...
    fn process_equalizer(&self, buffer: &mut crate::audio_buffer::AudioBuffer) {
        ...
        for (i, &sample) in data.iter().enumerate() {
            ...
            let mid_factor = if frequency >= 500.0 && frequency < 4000.0 { mid_gain } else { 1.0 };
    ...
    fn process_filter(&self, buffer: &mut crate::audio_buffer::AudioBuffer) {
        ...
        for i in 1..data.len() {
            match filter_type as i32 {
                0 => {
                    processed_data[i] = data[i] * (1.0 - normalized_cutoff) + processed_data[i-1] * normalized_cutoff;
                },
                1 => {
                    processed_data[i] = data[i] - processed_data[i-1] * normalized_cutoff;
                },
                2 => {
                    processed_data[i] = (data[i] - processed_data[i-1]) * normalized_cutoff + processed_data[i-1];
                },
