...
pub trait AudioFormatConverter: Send + Sync {
  ...
  ) -> Result<crate::audio_buffer::AudioBuffer, Box<dyn std::error::Error>>;
...
impl AudioConverter {
  ...
  pub fn set_input_format(&self, format: crate::audio_format::AudioFormatInfo) {
    ...
    *input_format = format;
    ...
    let _ = self
      .event_sender
      .send(ConverterEvent::FormatChanged(format));
  ...
  pub async fn convert(
    &self,
    input_buffer: &crate::audio_buffer::AudioBuffer,
  ) -> Result<crate::audio_buffer::AudioBuffer, Box<dyn std::error::Error>> {
    ...
    if let Some(ref mut conv) = *converter {
      ...
      match result {
        ...
        Err(e) => {
          ...
          let _ = self
            .event_sender
            .send(ConverterEvent::ConversionFailed(error_msg.clone()));
          Err(e)
        }
      }
    } else {
    ...
    F: Fn(ConversionProgress) + Send + Sync,
  {
    ...
    if let Some(ref mut conv) = *converter {
      ...
      for chunk_start in (0..input_buffer.length).step_by(chunk_size) {
        ...
        let chunk = input_buffer
          .get_slice(chunk_start, chunk_end)
        ...
        let progress = ConversionProgress {
          ...
          eta: if chunk_end > 0 {
            ...
            Some(std::time::Duration::from_secs_f64(
              remaining_frames as f64 / (chunk_end as f64 / 0.1),
            ))
          } else {
            None
          },
        ...
        let _ = self
          .event_sender
          .send(ConverterEvent::ConversionProgress(progress_percent));
  ...
  fn estimate_lossy_quality_loss(
    &self,
    input: &crate::audio_format::AudioFormatInfo,
    output: &crate::audio_format::AudioFormatInfo,
  ) -> Option<f32> {
