...
impl AudioEngine {
  ...
  pub async fn add_processor(
    ...
    let _ = self
      .event_sender
      .send(AudioEvent::ProcessorAdded(processor.id.clone()));
    Ok(())
  }
  ...
  pub async fn remove_processor(
    ...
    let _ = self
      .event_sender
      .send(AudioEvent::ProcessorRemoved(processor_id.to_string()));
    Ok(())
  }
  ...
  pub async fn create_stream(
  ...
  ) -> Result<Arc<AudioStream>, Box<dyn std::error::Error>> {
    ...
    let _ = self
      .event_sender
      .send(AudioEvent::StreamStarted(stream.id.clone()));
    Ok(stream)
  }
  ...
  pub async fn remove_stream(&self, stream_id: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(stream) = streams.remove(stream_id) {
      ...
      let _ = self
        .event_sender
        .send(AudioEvent::StreamStopped(stream_id.to_string()));
    }
    Ok(())
  }
