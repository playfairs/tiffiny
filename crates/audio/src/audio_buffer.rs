...
impl AudioBuffer {
  ...
  pub fn convert_format(&self, target_format: AudioFormat) -> AudioBuffer {
    ...
    let converted_data = match (&self.format, &target_format) {
      (AudioFormat::F32, AudioFormat::F64) => data
        .iter()
        .map(|&sample| sample as f64)
        .collect::<Vec<f64>>(),
      (AudioFormat::F32, AudioFormat::I16) => data
        .iter()
        .map(|&sample| (sample * i16::MAX as f32) as i16)
        .collect::<Vec<i16>>(),
      (AudioFormat::F32, AudioFormat::I24) => data
        .iter()
        .map(|&sample| (sample * ((1 << 23) - 1) as f32) as i32)
        .collect::<Vec<i32>>(),
      (AudioFormat::F32, AudioFormat::I32) => data
        .iter()
    ...
    let f32_data = match target_format {
      ...
      AudioFormat::I16 => converted_data
        .iter()
        .map(|&sample| sample as f32 / i16::MAX as f32)
        .collect(),
      AudioFormat::I24 => converted_data
        .iter()
        .map(|&sample| sample as f32 / ((1 << 23) - 1) as f32)
        .collect(),
      AudioFormat::I32 => converted_data
        .iter()
