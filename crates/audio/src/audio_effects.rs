...
impl AudioEffect {
    ...
    pub async fn process(&self, input_buffer: &mut crate::audio_buffer::AudioBuffer) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn process_chorus(&self, buffer: &mut crate::audio_buffer::AudioBuffer) {
        ...
        for voice in 0..voices {
            ...
            for i in 0..data.len() {
                ...
                let modulation = voice_depth * (2.0 * std::f32::consts::PI * voice_rate * time + phase_offset).sin();
    ...
    fn process_equalizer(&self, buffer: &mut crate::audio_buffer::AudioBuffer) {
        ...
        for (band_name, center_freq) in bands {
            let gain = parameters.get(&format!("{}_gain", band_name)).copied().unwrap_or(0.0);
            let q = parameters.get(&format!("{}_q", band_name)).copied().unwrap_or(1.0);
    ...
    fn process_filter(&self, buffer: &mut crate::audio_buffer::AudioBuffer) {
        ...
        match filter_type as i32 {
            0 => self.process_lowpass(&mut processed_data, normalized_cutoff, resonance, sample_rate),
            1 => self.process_highpass(&mut processed_data, normalized_cutoff, resonance, sample_rate),
            2 => self.process_bandpass(&mut processed_data, normalized_cutoff, resonance, sample_rate),
            _ => {}
        }
    ...
    fn mix_wet_dry(&self, wet_buffer: &mut crate::audio_buffer::AudioBuffer, dry_buffer: &crate::audio_buffer::AudioBuffer, mix: f32) {
