...
impl SessionManager {
  ...
  pub fn remove_open_file(&self, session_id: Uuid, file_path: &str) -> Result<()> {
    ...
    if let Some(session) = sessions.get_mut(&session_id) {
      session
        .state
        .open_files
        .retain(|f| f.file_path != file_path);
      session.last_accessed = std::time::SystemTime::now();
      Ok(())
    } else {
      Err(CoreError::Session(format!(
  ...
  pub fn add_recent_project(&self, session_id: Uuid, project: RecentProject) -> Result<()> {
    ...
    if let Some(session) = sessions.get_mut(&session_id) {
      session
        .state
        .recent_projects
        .retain(|p| p.project_id != project.project_id);
  ...
  pub fn cleanup_expired_sessions(&self) -> Result<usize> {
    let cutoff =
      std::time::SystemTime::now() - std::time::Duration::from_secs(self.session_timeout_seconds);
