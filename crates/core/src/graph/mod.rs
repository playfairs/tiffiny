...
impl GraphManager {
  ...
  pub fn remove_node(&self, graph_id: Uuid, node_id: Uuid) -> Result<()> {
    ...
    if let Some(graph) = graphs.get_mut(&graph_id) {
      ...
      graph
        .edges
        .retain(|_, edge| edge.source_node != node_id && edge.target_node != node_id);
      Ok(())
    } else {
      Err(CoreError::Task(format!("Graph {} not found", graph_id)))
    }
  ...
  fn check_cycles(&self, graph: &ProcessingGraph) -> Result<()> {
    ...
    for node_id in graph.nodes.keys() {
      if !visited.contains(node_id) {
        if self.has_cycle_dfs(node_id, &graph, &mut visited, &mut recursion_stack)? {
  ...
  async fn execute_node(graph: &ProcessingGraph, node_id: &Uuid) -> Result<serde_json::Value> {
    ...
    match node.node_type {
      NodeType::Input => Ok(serde_json::json!({"type": "input", "data": "raw_input"})),
      NodeType::Processor => Ok(serde_json::json!({"type": "processed", "node": node.name})),
      NodeType::Transform => Ok(serde_json::json!({"type": "transformed", "node": node.name})),
      NodeType::Filter => Ok(serde_json::json!({"type": "filtered", "node": node.name})),
      NodeType::Effect => Ok(serde_json::json!({"type": "effect_applied", "node": node.name})),
      NodeType::Analysis => Ok(serde_json::json!({"type": "analyzed", "node": node.name})),
      NodeType::Conversion => Ok(serde_json::json!({"type": "converted", "node": node.name})),
      NodeType::Output => Ok(serde_json::json!({"type": "output", "data": "final_result"})),
    }
  }
