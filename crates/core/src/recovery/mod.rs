...
impl RecoveryManager {
    ...
    pub fn cleanup_old_recovery_points(&self) -> Result<usize> {
        ...
        let cutoff_duration = std::time::Duration::from_secs(config.cleanup_old_after_days as u64 * 24 * 60 * 60);
    ...
    pub fn export_recovery_point(&self, recovery_id: Uuid, output_path: &Path) -> Result<()> {
        let recovery_point = self.get_recovery_point(recovery_id)
            .ok_or_else(|| CoreError::Recovery(format!("Recovery point {} not found", recovery_id)))?;
    ...
    pub fn import_recovery_point(&self, import_path: &Path) -> Result<Uuid> {
        let import_data: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(import_path)?)?;
        let recovery_point: RecoveryPoint = serde_json::from_value(import_data["recovery_point"].clone())?;
    ...
    fn save_recovery_point(&self, recovery_point: &RecoveryPoint) -> Result<()> {
        let config = self.config.read();
        let file_path = config.recovery_directory.join(format!("{}.recovery", recovery_point.id));
        drop(config);
