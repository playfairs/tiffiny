...
impl MemoryManager {
  ...
  pub fn cleanup_old_allocations(&self, older_than: std::time::Duration) -> Result<usize> {
    ...
    let allocations_to_remove: Vec<Uuid> = {
      ...
      allocations
        .values()
        .filter(|a| a.last_accessed.map(|last| last < cutoff).unwrap_or(false))
        .map(|a| a.id)
        .collect()
    };
  ...
  fn find_suitable_block(&self, pool: &MemoryPool, size_bytes: u64) -> Result<Uuid> {
    ...
    let suitable_blocks: Vec<_> = blocks
      .values()
      .filter(|block| block.pool_id == pool.id && !block.is_allocated && block.size >= size_bytes)
      .collect();
    ...
    let selected_block = match pool.allocation_strategy {
      AllocationStrategy::FirstFit => suitable_blocks.into_iter().min_by_key(|b| b.offset),
      AllocationStrategy::BestFit => suitable_blocks.into_iter().min_by_key(|b| b.size),
      AllocationStrategy::WorstFit => suitable_blocks.into_iter().max_by_key(|b| b.size),
      AllocationStrategy::BuddySystem => suitable_blocks.into_iter().min_by_key(|b| b.size),
    };
