...
pub type CommandHandler = Arc<dyn Fn(HashMap<String, serde_json::Value>) -> Result<CommandResult> + Send + Sync>;
...
impl CommandExecutor {
    ...
    pub async fn execute_command(
        ...
        if !command.enabled {
            return Err(CoreError::Task(format!("Command '{}' is disabled", command_name)));
        }
    ...
    fn validate_parameter_value(
        ...
        match param.parameter_type {
            ParameterType::String => {
                if !value.is_string() {
                    return Err(CoreError::Task(format!(
                        "Parameter '{}' must be a string", param.name
                    )));
                }
            },
            ParameterType::Integer => {
                if !value.is_i64() {
                    return Err(CoreError::Task(format!(
                        "Parameter '{}' must be an integer", param.name
                    )));
                }
            },
            ParameterType::Float => {
                if !value.is_number() {
                    return Err(CoreError::Task(format!(
                        "Parameter '{}' must be a number", param.name
                    )));
                }
            },
            ParameterType::Boolean => {
                if !value.is_boolean() {
                    return Err(CoreError::Task(format!(
                        "Parameter '{}' must be a boolean", param.name
                    )));
                }
            },
            _ => {}
        ...
        match validation {
            ...
            ParameterValidation::Range(min, max) => {
                if let Some(num) = value.as_f64() {
                    if num < *min || num > *max {
                        return Err(CoreError::Task(format!(
                            "Parameter '{}' must be between {} and {}", param.name, min, max
                        )));
                    }
                }
            },
            ParameterValidation::MinLength(min) => {
                if let Some(s) = value.as_str() {
                    if s.len() < *min {
                        return Err(CoreError::Task(format!(
                            "Parameter '{}' must be at least {} characters long", param.name, min
                        )));
                    }
                }
            },
            ParameterValidation::MaxLength(max) => {
                if let Some(s) = value.as_str() {
                    if s.len() > *max {
                        return Err(CoreError::Task(format!(
                            "Parameter '{}' must be at most {} characters long", param.name, max
                        )));
                    }
                }
            },
            ParameterValidation::Pattern(pattern) => {
                if let Some(s) = value.as_str() {
                    let regex = regex::Regex::new(pattern).map_err(|e| {
                        CoreError::Task(format!("Invalid regex pattern: {}", e))
                    })?;
                    if !regex.is_match(s) {
                        return Err(CoreError::Task(format!(
                            "Parameter '{}' does not match required pattern", param.name
                        )));
                    }
                }
            },
            ...
            ParameterValidation::Custom(validator) => {
                if !validator(value) {
                    return Err(CoreError::Task(format!(
                        "Parameter '{}' failed custom validation", param.name
                    )));
                }
            },
        }
    ...
    pub fn get_statistics(&self) -> CommandStatistics {
        ...
        for category in [
            ...
            let count = commands.values().filter(|cmd| cmd.category == category).count();
