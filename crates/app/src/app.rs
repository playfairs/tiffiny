use crate::{Bootstrap, AppState, EventBus, CommandExecutor, ShutdownHandler};
use tiffiny_core::{prelude::*, TaskManager, JobManager, GraphManager};
...
impl TiffinyApp {
    pub async fn new(bootstrap: Bootstrap) -> Result<Self> {
        ...
        let ui_manager = Arc::new(UiManager::new(
            state.clone(),
            event_bus.clone(),
            platform.clone(),
        ).await?);
    ...
    pub async fn shutdown(&mut self) -> Result<()> {
        *self.shutdown_requested.write() = true;
        ...
        self.task_manager.cleanup_completed_executions(std::time::Duration::from_secs(0))?;
        self.job_manager.cleanup_completed_jobs(std::time::Duration::from_secs(0))?;
    ...
    async fn process_tasks(&self) -> Result<()> {
        ...
        for task_execution in state.get_running_tasks() {
            if let Some(result) = self.task_manager.get_task_result(task_execution.task_id) {
                self.handle_task_completion(task_execution.task_id, result).await?;
            }
        }
    ...
    async fn handle_task_completion(&self, task_id: Uuid, result: tiffiny_core::task::TaskResult) -> Result<()> {
        if result.success {
    ...
    async fn create_new_project(&self) -> Result<()> {
        let project_id = self.state.create_project("Untitled Project".to_string()).await?;
    ...
    async fn export_file(&self, path: String, format: String) -> Result<()> {
        ...
        let event = crate::AppEvent::FileExported { path, format, export_id };
        self.event_bus.publish(event).await?;
