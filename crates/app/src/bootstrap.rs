...
impl Bootstrap {
  ...
  pub async fn perform_startup_checks(&self) -> Result<()> {
    ...
    if system_info.available_memory_mb < 1024 {
      tracing::warn!(
        "Low memory detected: {} MB available",
    ...
    if system_info.available_disk_space_gb < 10 {
      tracing::warn!(
        "Low disk space detected: {} GB available",
        system_info.available_disk_space_gb
      );
    }
    ...
    if !recovery_points.is_empty() {
      tracing::info(
        "Found {} recovery points from previous session",
        recovery_points.len(),
      );
      ...
      for recovery_point in &recovery_points {
        if matches!(
          ...
          tiffiny_core::recovery::RecoveryType::CrashRecovery
        ) {
          tracing::warn!(
            "Crash recovery point found from: {:?}",
  ...
  pub async fn create_memory_pools(&self) -> Result<()> {
    ...
    let pool_size = (self.config.memory_pool_size_mb * 1024 * 1024)
      .min((system_info.available_memory_mb / 2) * 1024 * 1024);
    ...
    tracing::info!(
      "Created memory pools: main={}, temp={}",
      main_pool_id,
      temp_pool_id
    );
