...
impl AppState {
  ...
  pub async fn create_project(&mut self, name: String) -> Result<Uuid> {
    let project_id = self
      .projects
      .create_project(name, "New project created from app state".to_string())?;

    if let Some(session_id) = self.current_session {
      self.projects.open_project(project_id)?;
      self
        .sessions
        .set_current_project(session_id, Some(project_id))?;
    }
  ...
  pub async fn save_project(&self, project_id: Uuid) -> Result<()> {
    ...
    self
      .projects
      .export_project(project_id, project_file.to_str().unwrap())?;
  ...
  pub fn update_task_progress(&mut self, execution_id: Uuid, progress: f32) -> Result<()> {
    if let Some(task_execution) = self
      .running_tasks
      .iter_mut()
  ...
  pub fn complete_task(
    ...
    self
      .running_tasks
      .retain(|te| te.execution_id != execution_id);
  ...
  fn determine_asset_type(&self, path: &str) -> Result<tiffiny_project::AssetType> {
    ...
    match extension.as_deref() {
      ...
      Some("tiffiny") => Ok(tiffiny_project::AssetType::Project),
      _ => Ok(tiffiny_project::AssetType::Raw),
    }
  }

  async fn extract_asset_metadata(
    &self,
    path: &str,
    asset_type: &tiffiny_project::AssetType,
  ) -> Result<tiffiny_project::AssetMetadata> {
