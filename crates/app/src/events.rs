...
impl EventBus {
  ...
  pub fn get_events_by_type(&self, event_type: &str) -> Vec<AppEvent> {
    ...
    history
      .iter()
      .filter(|event| {
        matches!(event, AppEvent::UiEvent(_)) && event_type == "ui"
  ...
  pub async fn wait_for_quit(&self) -> AppEvent {
    self
      .wait_for_event(|event| matches!(event, AppEvent::Quit))
      .await
  }
  ...
  pub fn get_statistics(&self) -> EventStatistics {
    ...
    for event in history.iter() {
      ...
      match event {
        ...
        AppEvent::ExportFile(..) => stats.file_events += 1,
