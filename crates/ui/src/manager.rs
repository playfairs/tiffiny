...
impl UiManager {
  ...
  async fn handle_key_press(&self, key: String, ctrl: bool, shift: bool, alt: bool) -> Result<()> {
    ...
    let event =
      crate::app::AppEvent::UiEvent(crate::app::UiEvent::KeyPressed(key, ctrl, shift, alt));
    ...
    egui::menu::bar(ui, |ui| {
      ...
      egui::menu::menu_button(ui, "View", |ui| {
        if ui.button("Timeline").clicked() {
          let event =
            crate::app::AppEvent::UiEvent(crate::app::UiEvent::PanelOpened("timeline".to_string()));
          let _ = self.event_bus.publish(event);
          ui.close_menu();
        }
        ...
        if ui.button("Waveform").clicked() {
          let event =
            crate::app::AppEvent::UiEvent(crate::app::UiEvent::PanelOpened("waveform".to_string()));
          let _ = self.event_bus.publish(event);
          ui.close_menu();
        }

        if ui.button("Spectrogram").clicked() {
          let event = crate::app::AppEvent::UiEvent(crate::app::UiEvent::PanelOpened(
