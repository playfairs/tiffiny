...
impl Layout {
  ...
  fn adjust_panel_positions(&mut self) {
    ...
    let menu_height = if self.menu_bar.visible {
      self.menu_bar.height
    } else {
      0
    };
    let status_height = if self.status_bar.visible {
  ...
  pub fn get_main_area_rect(&self) -> egui::Rect {
    ...
    egui::Rect::from_min_max(egui::pos2(x, y), egui::pos2(x + width, y + height))
  }
