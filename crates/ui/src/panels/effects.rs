...
impl EffectsPanel {
                ...
                presets: vec![
                    EffectPreset {
                        ...
                        parameters: {
                            ...
                            params.insert("xor_value".to_string(), serde_json::Value::Number(serde_json::Number::from(64)));
                            params.insert("apply_to_header".to_string(), serde_json::Value::Bool(false));
                            params
                        },
                    },
                    EffectPreset {
                        ...
                        parameters: {
                            let mut params = std::collections::HashMap::new();
                            params.insert("xor_value".to_string(), serde_json::Value::Number(serde_json::Number::from(200)));
            ...
            Effect {
                ...
                presets: vec![
                    EffectPreset {
                        ...
                        parameters: {
                            let mut params = std::collections::HashMap::new();
                            params.insert("intensity".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(0.2)));
                            params.insert("frame_skip".to_string(), serde_json::Value::Number(serde_json::Number::from(10)));
                            params
                        },
                    },
                ],
            ...
            Effect {
                ...
                presets: vec![
                    EffectPreset {
                        ...
                        parameters: {
                            let mut params = std::collections::HashMap::new();
                            params.insert("noise_level".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(0.1)));
                            params.insert("chromatic_aberration".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(0.05)));
                            params
                        },
                    },
                ],
                ...
                presets: vec![
                    EffectPreset {
                        ...
                        parameters: {
                            ...
                            params.insert("curvature".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(0.4)));
                            params
                        },
                    },
                ],
            ...
            Effect {
                ...
                parameters: vec![
                    ...
                    EffectParameter {
                        ...
                        parameter_type: ParameterType::Choice(vec!["Grayscale".to_string(), "RGB".to_string(), "HSV".to_string()]),
                ...
                presets: vec![
                    EffectPreset {
                        ...
                        parameters: {
                            let mut params = std::collections::HashMap::new();
                            params.insert("width".to_string(), serde_json::Value::Number(serde_json::Number::from(1920)));
                            params.insert("height".to_string(), serde_json::Value::Number(serde_json::Number::from(1080)));
                            params.insert("color_mode".to_string(), serde_json::Value::String("Grayscale".to_string()));
                            params
                        },
                    },
                ],
    ...
    pub fn apply_effect(&mut self, effect_id: &str) -> Result<String, String> {
        let effect = self.available_effects
            .iter()
    ...
    pub fn remove_effect(&mut self, instance_id: &str) -> bool {
        let index = self.applied_effects.iter().position(|e| e.instance_id == instance_id);
    ...
    pub fn toggle_effect(&mut self, instance_id: &str) -> bool {
        if let Some(effect) = self.applied_effects.iter_mut().find(|e| e.instance_id == instance_id) {
    ...
    pub fn reorder_effects(&mut self, instance_id: &str, new_order: usize) -> bool {
        ...
        if let Some(index) = index {
            if let Some(effect) = self.applied_effects.get(index) {
                ...
                self.applied_effects.remove(index);
                self.applied_effects.insert(new_order.min(self.applied_effects.len()), effect);
                true
            } else {
                false
            }
    ...
    pub fn update_effect_parameter(&mut self, instance_id: &str, parameter_id: &str, value: serde_json::Value) -> bool {
        if let Some(effect) = self.applied_effects.iter_mut().find(|e| e.instance_id == instance_id) {
    ...
    pub fn get_applied_effect(&self, instance_id: &str) -> Option<&AppliedEffect> {
        self.applied_effects.iter().find(|e| e.instance_id == instance_id)
    }
        ...
        self.available_effects
            ...
            .filter(|effect| {
                if !self.search_query.is_empty() && 
                   !effect.name.to_lowercase().contains(&self.search_query.to_lowercase()) &&
                   !effect.description.to_lowercase().contains(&self.search_query.to_lowercase()) {
                    return false;
                }
    ...
    pub fn render(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            egui::ComboBox::from_label("Category")
                .selected_text(format!("{:?}", self.category_filter.unwrap_or(EffectCategory::Audio)))
                .show_ui(ui, |ui| {
                    ui.selectable_value(&mut self.category_filter, None, "All");
                    ui.selectable_value(&mut self.category_filter, Some(EffectCategory::Audio), "Audio");
                    ui.selectable_value(&mut self.category_filter, Some(EffectCategory::Image), "Image");
                    ui.selectable_value(&mut self.category_filter, Some(EffectCategory::Video), "Video");
                    ui.selectable_value(&mut self.category_filter, Some(EffectCategory::DataBend), "DataBend");
                    ui.selectable_value(&mut self.category_filter, Some(EffectCategory::Glitch), "Glitch");
                    ui.selectable_value(&mut self.category_filter, Some(EffectCategory::Analog), "Analog");
                    ui.selectable_value(&mut self.category_filter, Some(EffectCategory::Color), "Color");
                    ui.selectable_value(&mut self.category_filter, Some(EffectCategory::Filter), "Filter");
                    ui.selectable_value(&mut self.category_filter, Some(EffectCategory::Transform), "Transform");
                });
        ...
        egui::Splitter::horizontal(&mut 200.0)
            .show(ui, |ui| {
    ...
    fn render_effect_item(&mut self, ui: &mut egui::Ui, effect: &Effect) {
        let is_selected = self.selected_effect.as_ref().map_or(false, |id| id == &effect.id);
        ...
        ui.horizontal(|ui| {
            ...
            ui.vertical(|ui| {
                ...
                ui.label(egui::RichText::new(&effect.description).size(10.0).color(egui::Color32::from_rgb(160, 160, 160)));
            });
        });
        ...
        if ui.button("Apply").clicked() {
            if let Ok(instance_id) = self.apply_effect(&effect.id) {
                tracing::info!("Applied effect: {} with instance ID: {}", effect.name, instance_id);
            }
        }
        ...
        if self.show_presets && !effect.presets.is_empty() {
            ...
            for preset in &effect.presets {
                if ui.button(&preset.name).clicked() {
                    tracing::info!("Applied preset: {} for effect: {}", preset.name, effect.name);
                }
            }
        }
    }
    ...
    fn render_applied_effect_item(&mut self, ui: &mut egui::Ui, effect: &mut AppliedEffect, index: usize) {
    ...
    fn render_effect_parameters(&mut self, ui: &mut egui::Ui, original_effect: &Effect, applied_effect: &mut AppliedEffect) {
        for parameter in &original_effect.parameters {
            let current_value = applied_effect.parameters.get(&parameter.id).cloned().unwrap_or(parameter.default_value.clone());
            ...
            ui.horizontal(|ui| {
                        ...
                        if let Some(min) = parameter.min_value {
                            if let Some(max) = parameter.max_value {
                                if let Some(value) = new_value.as_f64() {
                                    ...
                                    if ui.add(egui::Slider::new(&mut float_val, min..=max)).changed() {
                                        new_value = serde_json::Value::Number(serde_json::Number::from_f64(float_val as f64));
                                        changed = true;
                                    }
                                }
                            }
                        ...
                        if let Some(min) = parameter.min_value {
                            if let Some(max) = parameter.max_value {
                                if let Some(value) = new_value.as_i64() {
                                    ...
                                    if ui.add(egui::Slider::new(&mut int_val, min as i32..=max as i32)).changed() {
                ...
                if changed {
                    applied_effect.parameters.insert(parameter.id.clone(), new_value);
            ...
            if !parameter.description.is_empty() {
                ...
                if ui.is_item_hovered() {
                    egui::show_tooltip_at_pointer(ui.ctx(), || {
                        egui::Label::new(&parameter.description)
                    });
                }
            }
        }
