...
impl ProjectExplorerPanel {
  ...
  fn filter_and_sort_assets(&self) -> Vec<&AssetNode> {
    let mut filtered: Vec<&AssetNode> = self
      .assets
      .iter()
      .filter(|asset| {
  ...
  pub fn render(&mut self, ui: &mut egui::Ui) {
    ui.horizontal(|ui| {
      ui.add(egui::TextEdit::singleline(&mut self.search_query).hint_text("Search assets..."));
      ...
      egui::ComboBox::from_label("Filter")
        .selected_text(format!(
          "{:?}",
          self.filter_type.unwrap_or(AssetType::Audio)
        ))
        .show_ui(ui, |ui| {
  ...
  fn render_thumbnail_item(&mut self, ui: &mut egui::Ui, asset: &AssetNode, size: f32) {
    ui.vertical(|ui| {
      ...
      let (rect, _) = ui.allocate_exact_size(egui::vec2(size, size + 30.0), egui::Sense::click());

      let painter = ui.painter();
      painter.rect_stroke(rect, 4.0, egui::Stroke::new(2.0, frame_color));
  ...
  fn show_asset_properties(&self) {
    tracing::info!(
      "Showing properties for {} selected assets",
