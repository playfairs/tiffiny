...
impl ExportPanel {
  ...
  fn apply_quality_preset(&mut self) {
    match self.quality_preset {
      QualityPreset::Low => {
        ...
        if matches!(
          self.export_settings.video_codec,
          VideoCodec::H264 | VideoCodec::H265
        ) {
          self
            .export_settings
            .custom_parameters
            .insert("crf".to_string(), "28".to_string());
        }
      }
      QualityPreset::Medium => {
        ...
        if matches!(
          ...
          self
            .export_settings
            .custom_parameters
            .insert("crf".to_string(), "23".to_string());
        }
      }
      QualityPreset::High => {
        ...
        if matches!(
          ...
          self
            .export_settings
            .custom_parameters
            .insert("crf".to_string(), "18".to_string());
        }
      }
      QualityPreset::Ultra => {
        ...
        if matches!(
          ...
          self
            .export_settings
            .custom_parameters
            .insert("crf".to_string(), "15".to_string());
        }
      }
      QualityPreset::Lossless => {
        ...
        self
          .export_settings
          .custom_parameters
          .insert("crf".to_string(), "0".to_string());
      }
  ...
  fn render_export_settings(&mut self, ui: &mut egui::Ui) {
    egui::CollapsingHeader::new("Advanced Settings")
      .default_open(false)
      .show(ui, |ui| match self.export_format {
        ExportFormat::Video => {
  ...
  fn render_video_settings(&mut self, ui: &mut egui::Ui) {
    ...
    ui.horizontal(|ui| {
      ...
      egui::ComboBox::from_label("")
        ...
        .show_ui(ui, |ui| {
          ui.selectable_value(
            &mut self.export_settings.video_codec,
            VideoCodec::H264,
            "H.264",
          );
          ui.selectable_value(
    ...
    ui.horizontal(|ui| {
      ...
      egui::ComboBox::from_label("")
        ...
        .show_ui(ui, |ui| {
          ui.selectable_value(
            &mut self.export_settings.audio_codec,
            AudioCodec::PCM,
            "PCM",
          );
          ui.selectable_value(
  ...
  fn browse_output_path(&self) -> Option<String> {
    match self.export_format {
      ExportFormat::Video => rfd::FileDialog::new()
        .add_filter("MP4", &["mp4"])
        .add_filter("MKV", &["mkv"])
        .add_filter("MOV", &["mov"])
        .save_file(),
      ExportFormat::Audio => rfd::FileDialog::new()
        .add_filter("WAV", &["wav"])
        .add_filter("FLAC", &["flac"])
        .add_filter("MP3", &["mp3"])
        .save_file(),
      ExportFormat::Image => rfd::FileDialog::new()
        .add_filter("PNG", &["png"])
        .add_filter("TIFF", &["tiff"])
        .add_filter("JPEG", &["jpg", "jpeg"])
        .save_file(),
      ExportFormat::ImageSequence => rfd::FileDialog::new()
        .pick_folder()
        .map(|path| format!("{}/sequence_%04d.png", path.to_string_lossy())),
      ExportFormat::Raw => rfd::FileDialog::new()
        .add_filter("RAW", &["raw"])
        .add_filter("BIN", &["bin"])
        .save_file(),
      ExportFormat::Project => rfd::FileDialog::new()
  ...
  fn preview_export(&self) {
    tracing::info!(
      "Previewing export with settings: {:?}",
