...
impl VideoLoader {
    ...
    pub async fn load_from_path(&self, path: &str, options: Option<LoadOptions>) -> Result<Arc<crate::video_buffer::Buffer>, Box<dyn std::error::Error>> {
        let _ = self.event_sender.send(LoaderEvent::LoadingStarted(path.to_string()));
        
        let load_options = options.unwrap_or_default();
        
        let video_info = self.get_video_info(path).await?;
        let video_buffer = self.create_buffer_from_info(&video_info, &load_options).await?;

        let _ = self.event_sender.send(LoaderEvent::LoadingCompleted(Arc::new(video_buffer)));
        
        Ok(Arc::new(video_buffer))
    }

    pub async fn load_from_url(&self, url: &str, options: Option<LoadOptions>) -> Result<Arc<crate::video_buffer::Buffer>, Box<dyn std::error::Error>> {
        let _ = self.event_sender.send(LoaderEvent::LoadingStarted(url.to_string()));
    ...
    pub async fn load_from_bytes(&self, data: &[u8], options: Option<LoadOptions>) -> Result<Arc<crate::video_buffer::Buffer>, Box<dyn std::error::Error>> {
        let _ = self.event_sender.send(LoaderEvent::LoadingStarted("From bytes".to_string()));
    ...
    pub async fn load_batch(&self, paths: &[String], options: Option<LoadOptions>) -> Result<Vec<Arc<crate::video_buffer::Buffer>>, Box<dyn std::error::Error>> {
        let mut results = Vec::new();
        
        for (index, path) in paths.iter().enumerate() {
            let progress = (index as f32 / paths.len() as f32) * 100.0;
            let _ = self.event_sender.send(LoaderEvent::LoadingProgress(progress));
    ...
    pub async fn get_video_info_from_url(&self, url: &str) -> Result<VideoInfo, Box<dyn std::error::Error>> {
        self.get_video_info(url).await
    }

    pub async fn get_video_info_from_bytes(&self, data: &[u8]) -> Result<VideoInfo, Box<dyn std::error::Error>> {
    ...
    async fn create_buffer_from_info(&self, info: &VideoInfo, options: &LoadOptions) -> Result<crate::video_buffer::Buffer, Box<dyn std::error::Error>> {
        ...
        let mut video_buffer = crate::video_buffer::Buffer::new(
            target_width,
            target_height,
            pixel_format,
            info.frame_rate,
    ...
    pub async fn load_thumbnail(&self, path: &str, width: u32, height: u32) -> Result<crate::video_buffer::VideoFrame, Box<dyn std::error::Error>> {
        let video_buffer = self.load_from_path(path, Some(LoadOptions {
            frame_count: Some(1),
    ...
    pub async fn load_keyframes(&self, path: &str, interval: u32) -> Result<Vec<crate::video_buffer::VideoFrame>, Box<dyn std::error::Error>> {
        let video_buffer = self.load_from_path(path, None).await?;
        Ok(video_buffer.extract_keyframes(interval))
    }

    pub async fn load_segment(&self, path: &str, start_time: std::time::Duration, duration: std::time::Duration) -> Result<Arc<crate::video_buffer::Buffer>, Box<dyn std::error::Error>> {
        let video_info = self.get_video_info(path).await?;
    ...
    pub async fn load_with_audio(&self, path: &str) -> Result<(Arc<crate::video_buffer::Buffer>, Option<Arc<tiffiny_audio::audio_buffer::AudioBuffer>>), Box<dyn std::error::Error>> {
        let video_buffer = self.load_from_path(path, Some(LoadOptions {
            extract_audio: true,
            ..Default::default()
        })).await?;
    ...
    async fn extract_audio_from_video(&self, path: &str) -> Result<Option<Arc<tiffiny_audio::audio_buffer::AudioBuffer>>, Box<dyn std::error::Error>> {
    ...
    pub async fn load_subtitles(&self, path: &str) -> Result<Vec<String>, Box<dyn std::error::Error>> {
        Ok(vec![
    ...
    pub async fn load_chapters(&self, path: &str) -> Result<Vec<crate::video_buffer::Chapter>, Box<dyn std::error::Error>> {
        Ok(vec![
    ...
    pub fn get_file_info(&self, path: &str) -> Result<std::collections::HashMap<String, String>, Box<dyn std::error::Error>> {
    ...
    pub async fn load_with_progress<F>(&self, path: &str, options: Option<LoadOptions>, progress_callback: F) -> Result<Arc<crate::video_buffer::Buffer>, Box<dyn std::error::Error>>
    where
        F: Fn(f32) + Send + Sync,
    {
        let _ = self.event_sender.send(LoaderEvent::LoadingStarted(path.to_string()));
        ...
        for i in 0..=100 {
            ...
            let _ = self.event_sender.send(LoaderEvent::LoadingProgress(i as f32));
