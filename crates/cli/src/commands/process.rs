...
pub async fn handle(command: ProcessCommands, config: &Config) -> Result<()> {
    match command.command {
        ProcessSubCommand::File { input, output, operation, params } => {
            process_file(input, output, operation, params, config).await
        },
        ProcessSubCommand::Batch { input, output, operation, params, parallel, workers } => {
            process_batch(input, output, operation, params, parallel, workers, config).await
        },
        ProcessSubCommand::Pipeline { config, input, output, dry_run } => {
            process_pipeline(config, input, output, dry_run, config).await
        },
        ProcessSubCommand::List { media_type, detailed } => {
            list_operations(media_type, detailed, config).await
        },
        ProcessSubCommand::Info { operation } => {
            show_operation_info(operation, config).await
        },
    }
}

async fn process_file(input: PathBuf, output: PathBuf, operation: String, params: Option<String>, config: &Config) -> Result<()> {
    println!("START");
...
async fn process_batch(input: String, output: PathBuf, operation: String, params: Option<String>, parallel: bool, workers: usize, config: &Config) -> Result<()> {
    println!("START");
...
async fn process_pipeline(config: PathBuf, input: String, output: PathBuf, dry_run: bool, config_data: &Config) -> Result<()> {
    println!("START");
...
async fn list_operations(media_type: Option<String>, detailed: bool, config: &Config) -> Result<()> {
    let operations = vec![
...
async fn process_files_sequential(files: Vec<PathBuf>, output_dir: &PathBuf, operation: &str, params: Option<&str>, config: &Config) -> Result<()> {
...
async fn process_files_parallel(files: Vec<PathBuf>, output_dir: &PathBuf, operation: &str, params: Option<&str>, workers: usize, config: &Config) -> Result<()> {
    use indicatif::{ProgressBar, ProgressStyle};
...
async fn process_single_file(input: &PathBuf, output: &PathBuf, operation: &str, params: Option<&str>, config: &Config) -> Result<()> {
...
fn generate_output_filename(input: &PathBuf, operation: &str) -> String {
    if let Some(stem) = input.file_stem() {
        if let Some(extension) = input.extension() {
            format!("{}_{}.{}", stem.to_string_lossy(), operation, extension.to_string_lossy())
        } else {
