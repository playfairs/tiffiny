...
pub async fn handle(command: ConvertCommands, config: &Config) -> Result<()> {
  match command.command {
    ...
    ConvertSubCommand::Pipeline {
      config,
      input,
      output,
      dry_run,
    } => convert_pipeline(config, input, output, dry_run, config).await,
    ConvertSubCommand::Formats {
      media_type,
      detailed,
    } => list_formats(media_type, detailed, config).await,
    ConvertSubCommand::Options {
      input_format,
      output_format,
    } => show_options(input_format, output_format, config).await,
    ConvertSubCommand::Analyze { input, detailed } => analyze_file(input, detailed, config).await,
    ConvertSubCommand::Preset { command } => handle_preset(command, config).await,
  }
}
...
async fn list_formats(media_type: Option<String>, detailed: bool, config: &Config) -> Result<()> {
  ...
  let filtered_formats: Vec<_> = formats
    .iter()
    .filter(|(_, _, mt)| {
...
async fn handle_preset(command: PresetCommand, config: &Config) -> Result<()> {
  match command {
    ...
    PresetCommand::Show { name } => show_preset(name, config).await,
    PresetCommand::Apply {
      name,
      input,
      output,
    } => apply_preset(name, input, output, config).await,
    PresetCommand::Delete { name } => delete_preset(name, config).await,
  }
}
...
async fn convert_files_sequential(
  files: Vec<PathBuf>,
  output_dir: &PathBuf,
  format: &str,
  quality: Option<u8>,
...
async fn convert_files_parallel(
  files: Vec<PathBuf>,
  output_dir: &PathBuf,
  format: &str,
  quality: Option<u8>,
