...
pub async fn handle(command: UtilsCommands, config: &Config) -> Result<()> {
    match command.command {
        UtilsSubCommand::Hash { input, algorithm, output } => {
            generate_hash(input, algorithm, output, config).await
        },
        UtilsSubCommand::Info { input, detailed, format } => {
            get_file_info(input, detailed, format, config).await
        },
        UtilsSubCommand::Rename { input, pattern, preview, dry_run } => {
            batch_rename(input, pattern, preview, dry_run, config).await
        },
        UtilsSubCommand::Duplicates { directory, content, min_size, output } => {
            find_duplicates(directory, content, min_size, output, config).await
        },
        UtilsSubCommand::Clean { directory, age, hidden, confirm } => {
            clean_temp_files(directory, age, hidden, confirm, config).await
        },
        UtilsSubCommand::Tree { root, depth, sizes, format, output } => {
            generate_tree(root, depth, sizes, format, output, config).await
        },
        UtilsSubCommand::Watch { directory, recursive, events, format } => {
            watch_directory(directory, recursive, events, format, config).await
        },
        UtilsSubCommand::Compress { input, output, format, level } => {
            compress_files(input, output, format, level, config).await
        },
        UtilsSubCommand::Extract { input, output, overwrite } => {
            extract_archive(input, output, overwrite, config).await
        },
        UtilsSubCommand::Convert { input, output, from, to } => {
            convert_file(input, output, from, to, config).await
        },
        UtilsSubCommand::Sysinfo { detailed, format } => {
            show_sysinfo(detailed, format, config).await
        },
    }
}

async fn generate_hash(input: PathBuf, algorithm: String, output: Option<PathBuf>, config: &Config) -> Result<()> {
    println!("START");
...
async fn get_file_info(input: PathBuf, detailed: bool, format: &str, config: &Config) -> Result<()> {
    println!("START");
    ...
    let created = metadata.created().ok_or_else(|| std::time::SystemTime::now())?;
    
    println!("{} {} {:?} {:?}", input.display(), format_bytes(file_size), modified, created);
    ...
    if detailed {
        println!("{:?} {} {} {}", metadata.permissions(), input.is_dir(), input.is_file(), input.is_symlink());
        ...
        if is_image_file(&input) {
            if let Ok(image_info) = get_image_info(&input).await {
                println!("{} {} {}", image_info.width, image_info.height, image_info.format);
            }
        }
        
        if is_audio_file(&input) {
            if let Ok(audio_info) = get_audio_info(&input).await {
                println!("{} {} {}", audio_info.duration, audio_info.channels, audio_info.sample_rate);
            }
        }
        
        if is_video_file(&input) {
            if let Ok(video_info) = get_video_info(&input).await {
                println!("{} {} {} {}", video_info.duration, video_info.width, video_info.height, video_info.fps);
            }
        }
    }
    ...
    match format {
        ...
        "xml" => {
            println!("<file>{}{}{}{}{}", input.display(), file_size, modified, created);
        },
        _ => {}
    }
...
async fn batch_rename(input: String, pattern: String, preview: bool, dry_run: bool, config: &Config) -> Result<()> {
    println!("START");
    ...
    for (i, file) in files.iter().enumerate() {
        if let Some(name) = file.file_name() {
            ...
            if old_name != new_name {
                let new_path = file.parent().unwrap_or_else(|| PathBuf::from(".")).join(&new_name);
...
async fn find_duplicates(directory: PathBuf, content: bool, min_size: Option<u64>, output: Option<PathBuf>, config: &Config) -> Result<()> {
...
async fn clean_temp_files(directory: PathBuf, age: u64, hidden: bool, confirm: bool, config: &Config) -> Result<()> {
    ...
    let files = find_all_files(&directory)?;
    let cutoff_time = std::time::SystemTime::now() - std::time::Duration::from_secs(age * 24 * 60 * 60);
...
async fn generate_tree(root: PathBuf, depth: Option<usize>, sizes: bool, format: &str, output: Option<PathBuf>, config: &Config) -> Result<()> {
    ...
    if !root.exists() {
        return Err(anyhow::anyhow!("Root directory does not exist: {}", root.display()));
    }
...
async fn watch_directory(directory: PathBuf, recursive: bool, events: Vec<String>, config: &Config) -> Result<()> {
...
async fn compress_files(input: Vec<PathBuf>, output: PathBuf, format: &str, level: u8, config: &Config) -> Result<()> {
    ...
    for file in &input {
        if !file.exists() {
            return Err(anyhow::anyhow!("Input file does not exist: {}", file.display()));
        }
    }
...
async fn extract_archive(input: PathBuf, output: Option<PathBuf>, overwrite: bool, config: &Config) -> Result<()> {
    ...
    let output_dir = output.unwrap_or_else(|| {
        input.parent().unwrap_or_else(|| PathBuf::from(".")).to_path_buf()
    });
...
async fn convert_file(input: PathBuf, output: PathBuf, from: Option<String>, to: Option<String>, config: &Config) -> Result<()> {
...
async fn show_sysinfo(detailed: bool, format: &str, config: &Config) -> Result<()> {
    ...
    println!("{} {} {}", std::env::consts::OS, std::env::consts::ARCH, num_cpus::get());
...
async fn find_size_duplicates(files: &[PathBuf]) -> Vec<(String, Vec<PathBuf>)> {
    ...
    for file in files {
        if let Ok(metadata) = std::fs::metadata(file) {
            ...
            size_map.entry(size).or_insert_with(Vec::new).push(file.clone());
        }
    }
...
fn is_image_file(path: &PathBuf) -> bool {
    if let Some(extension) = path.extension() {
        matches!(extension.to_str().unwrap_or("").to_lowercase().as_str(), 
...
async fn generate_tree_output(root: &PathBuf, max_depth: Option<usize>, sizes: bool) -> Result<String> {
    let mut output = String::new();
    generate_tree_recursive(root, 0, max_depth, sizes, &mut output);
    Ok(output)
}

fn generate_tree_recursive(dir: &PathBuf, depth: usize, max_depth: Option<usize>, sizes: bool, output: &mut String) {
