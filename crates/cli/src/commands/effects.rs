...
pub async fn handle(command: EffectsCommands, config: &Config) -> Result<()> {
  match command.command {
    ...
    EffectsSubCommand::CompressionArtifacts {
      input,
      output,
      format,
      quality,
    ...
    EffectsSubCommand::ColorManipulation {
      input,
      output,
      manipulation,
      amount,
    ...
    EffectsSubCommand::TextureSynthesis {
      input,
      output,
      synthesis_type,
      size,
    ...
    EffectsSubCommand::Info { effect } => show_effect_info(effect, config).await,
    EffectsSubCommand::Preview {
      input,
      effect,
      params,
      size,
...
async fn apply_compression_artifacts(
  input: PathBuf,
  output: PathBuf,
  format: String,
  quality: u8,
...
async fn apply_error_diffusion(
  input: PathBuf,
  output: PathBuf,
  diffusion_type: String,
  colors: u8,
...
async fn apply_procedural_noise(
  input: PathBuf,
  output: PathBuf,
  noise_type: String,
  scale: f32,
...
async fn apply_color_manipulation(
  input: PathBuf,
  output: PathBuf,
  manipulation: String,
  amount: f32,
...
async fn apply_texture_synthesis(
  input: PathBuf,
  output: PathBuf,
  synthesis_type: String,
  size: Option<String>,
