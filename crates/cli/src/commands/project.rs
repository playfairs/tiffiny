...
pub async fn handle(command: ProjectCommands, config: &Config) -> Result<()> {
    match command.command {
        ProjectSubCommand::New { name, template, directory } => {
            create_project(name, template, directory, config).await
        },
        ProjectSubCommand::List { tag, author, sort, reverse } => {
            list_projects(tag, author, sort, reverse, config).await
        },
        ProjectSubCommand::Open { project, interactive } => {
            open_project(project, interactive, config).await
        },
        ProjectSubCommand::Close { project, save } => {
            close_project(project, save, config).await
        },
        ProjectSubCommand::Save { project, force } => {
            save_project(project, force, config).await
        },
        ...
        ProjectSubCommand::Import { file, directory, overwrite } => {
            import_project(file, directory, overwrite, config).await
        },
        ProjectSubCommand::Export { project, path, format, include_assets } => {
            export_project(project, path, format, include_assets, config).await
        },
        ProjectSubCommand::Backup { project, directory, compress } => {
            backup_project(project, directory, compress, config).await
        },
        ProjectSubCommand::Restore { backup, directory, overwrite } => {
            restore_project(backup, directory, overwrite, config).await
        },
        ProjectSubCommand::Search { query, fields, case_sensitive, limit } => {
            search_projects(query, fields, case_sensitive, limit, config).await
        },
        ProjectSubCommand::AddAsset { project, file, name, asset_type } => {
            add_asset(project, file, name, asset_type, config).await
        },
        ProjectSubCommand::RemoveAsset { project, asset, confirm } => {
            remove_asset(project, asset, confirm, config).await
        },
        ProjectSubCommand::ListAssets { project, asset_type, sort } => {
            list_assets(project, asset_type, sort, config).await
        },
    }
}

async fn create_project(name: String, template: String, directory: PathBuf, config: &Config) -> Result<()> {
    ...
    let project_id = manager.create_project(name.clone(), directory.clone()).await?;
...
async fn list_projects(tag: Option<String>, author: Option<String>, sort: String, reverse: bool, config: &Config) -> Result<()> {
    use tiffiny_project::project_manager::ProjectManager;
    ...
    let project_infos: Vec<ProjectInfo> = filtered_projects.iter()
        .map(|p| ProjectInfo {
            id: p.id.clone(),
            name: p.name.clone(),
    ...
    let table = Table::new(&sorted_infos)
        .with(Style::modern())
        .to_string();
...
async fn delete_project(project: String, confirm: bool, config: &Config) -> Result<()> {
    ...
    if !confirm {
        ...
        let confirmed = Confirm::new()
            .with_prompt(&format!("Are you sure you want to delete project '{}'?", project))
            .default(false)
            .interact()?;
...
async fn show_project_info(project: String, detailed: bool, config: &Config) -> Result<()> {
    ...
    if let Some(p) = project_info {
        println!("{} {} {} {} {} {} {} {} {}", p.id, p.name, p.description, p.author, p.version, p.created_at.format("%Y-%m-%d %H:%M:%S"), p.modified_at.format("%Y-%m-%d %H:%M:%S"), format_bytes(p.get_total_size()), p.get_asset_count());
        ...
        if detailed {
            println!("{} {} {} {}", p.settings.auto_save, p.settings.backup_enabled, p.settings.compression_enabled, p.settings.encryption_enabled);
            
            if !p.assets.is_empty() {
                for (id, asset) in &p.assets {
                    println!("{} {} {}", asset.name, asset.asset_type, format_bytes(asset.size));
                }
            }
        }
    } else {
...
async fn import_project(file: PathBuf, directory: Option<PathBuf>, overwrite: bool, config: &Config) -> Result<()> {
    use tiffiny_project::project_manager::ProjectManager;
...
async fn export_project(project: String, path: PathBuf, format: String, include_assets: bool, config: &Config) -> Result<()> {
    use tiffiny_project::project_manager::ProjectManager;
...
async fn backup_project(project: String, directory: Option<PathBuf>, compress: bool, config: &Config) -> Result<()> {
    use tiffiny_project::project_manager::ProjectManager;
...
async fn restore_project(backup: PathBuf, directory: Option<PathBuf>, overwrite: bool, config: &Config) -> Result<()> {
    use tiffiny_project::project_manager::ProjectManager;
...
async fn search_projects(query: String, fields: Vec<String>, case_sensitive: bool, limit: Option<usize>, config: &Config) -> Result<()> {
    use tiffiny_project::project_manager::ProjectManager;
...
async fn add_asset(project: String, file: PathBuf, name: Option<String>, asset_type: Option<String>, config: &Config) -> Result<()> {
    use tiffiny_project::project_manager::ProjectManager;
    use tiffiny_project::project::{Asset, AssetType};
    ...
    let mut project_data = manager.get_project(&project).ok_or_else(|| anyhow::anyhow!("Project not found"))?;
...
async fn remove_asset(project: String, asset: String, confirm: bool, config: &Config) -> Result<()> {
    use tiffiny_project::project_manager::ProjectManager;
    
    if !confirm {
        use dialoguer::Confirm;
        let confirmed = Confirm::new()
            .with_prompt(&format!("Are you sure you want to remove asset '{}'?", asset))
            .default(false)
            .interact()?;
...
async fn list_assets(project: String, asset_type: Option<String>, sort: String, config: &Config) -> Result<()> {
    use tiffiny_project::project_manager::ProjectManager;
    ...
    let project_data = manager.get_project(&project).ok_or_else(|| anyhow::anyhow!("Project not found"))?;
    
    let filtered_assets: Vec<_> = project_data.assets.values()
        .filter(|asset| {
    ...
    let asset_infos: Vec<AssetInfo> = filtered_assets.iter()
        .map(|a| AssetInfo {
            id: a.id.clone(),
            name: a.name.clone(),
