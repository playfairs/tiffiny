...
pub async fn handle(command: ConfigCommands, config: &Config) -> Result<()> {
  match command.command {
    ...
    ConfigSubCommand::Set { key, value, file } => set_config(key, value, file, config).await,
    ConfigSubCommand::Get { key, file, format } => get_config(key, file, &format, config).await,
    ConfigSubCommand::Reset { section, confirm } => reset_config(section, confirm, config).await,
    ConfigSubCommand::Validate { file } => validate_config(file, config).await,
    ConfigSubCommand::Edit { file, editor } => edit_config(file, editor, config).await,
...
async fn init_config(
  ...
  if config_dir.exists() && !overwrite {
    return Err(anyhow::anyhow!(
      "Configuration directory already exists: {}. Use --overwrite to replace.",
      config_dir.display()
    ));
  }
