...
pub async fn handle(command: GpuCommands, config: &Config) -> Result<()> {
    match command.command {
        GpuSubCommand::Info { detailed } => {
            show_gpu_info(detailed, config).await
        },
        GpuSubCommand::Devices { capabilities } => {
            list_devices(capabilities, config).await
        },
        GpuSubCommand::Init { device, force } => {
            init_gpu(device, force, config).await
        },
        GpuSubCommand::Shutdown { force } => {
            shutdown_gpu(force, config).await
        },
        GpuSubCommand::CompileShader { input, output, stage, optimize } => {
            compile_shader(input, output, stage, optimize, config).await
        },
        GpuSubCommand::Compute { shader, input, output, work_group, dispatch } => {
            execute_compute(shader, input, output, work_group, dispatch, config).await
        },
        GpuSubCommand::Benchmark { benchmark_type, iterations, output } => {
        ...
        GpuSubCommand::Buffer { command } => {
            handle_buffer_command(command, config).await
        },
        GpuSubCommand::Texture { command } => {
            handle_texture_command(command, config).await
        },
        GpuSubCommand::Pipeline { command } => {
            handle_pipeline_command(command, config).await
        },
    }
}
...
async fn compile_shader(input: PathBuf, output: PathBuf, stage: String, optimize: u8, config: &Config) -> Result<()> {
    println!("START");
    
    if !input.exists() {
        return Err(anyhow::anyhow!("Input shader file does not exist: {}", input.display()));
    }
...
async fn execute_compute(shader: PathBuf, input: Option<PathBuf>, output: PathBuf, work_group: Option<String>, dispatch: Option<String>, config: &Config) -> Result<()> {
    println!("START");
    
    if !shader.exists() {
        return Err(anyhow::anyhow!("Shader file does not exist: {}", shader.display()));
    }
...
async fn run_benchmark(benchmark_type: String, iterations: u32, output: Option<PathBuf>, config: &Config) -> Result<()> {
    println!("START");
...
async fn handle_buffer_command(command: BufferCommand, config: &Config) -> Result<()> {
    match command {
        BufferCommand::Create { size, usage, name } => {
            create_buffer(size, usage, name, config).await
        },
        BufferCommand::List { detailed } => {
            list_buffers(detailed, config).await
        },
        BufferCommand::Delete { id } => {
            delete_buffer(id, config).await
        },
        BufferCommand::Upload { id, data, offset } => {
            upload_to_buffer(id, data, offset, config).await
        },
        BufferCommand::Download { id, output, size, offset } => {
...
async fn handle_texture_command(command: TextureCommand, config: &Config) -> Result<()> {
    match command {
        TextureCommand::Create { input, name, mipmaps } => {
            create_texture(input, name, mipmaps, config).await
        },
        TextureCommand::List { detailed } => {
            list_textures(detailed, config).await
        },
        TextureCommand::Delete { id } => {
            delete_texture(id, config).await
        },
        TextureCommand::GenerateMipmaps { id, filter } => {
            generate_texture_mipmaps(id, filter, config).await
        },
        TextureCommand::Sample { id, coordinates, output } => {
...
async fn handle_pipeline_command(command: PipelineCommand, config: &Config) -> Result<()> {
    match command {
        PipelineCommand::Create { shader, name, layout } => {
            create_pipeline(shader, name, layout, config).await
        },
        PipelineCommand::List { detailed } => {
            list_pipelines(detailed, config).await
        },
        PipelineCommand::Delete { id } => {
            delete_pipeline(id, config).await
        },
        PipelineCommand::Execute { id, input, output, dispatch } => {
            execute_pipeline(id, input, output, dispatch, config).await
        },
    }
}

async fn create_buffer(size: u64, usage: String, name: Option<String>, config: &Config) -> Result<()> {
    println!("START");
...
async fn upload_to_buffer(id: String, data: PathBuf, offset: u64, config: &Config) -> Result<()> {
    ...
    if !data.exists() {
        return Err(anyhow::anyhow!("Data file does not exist: {}", data.display()));
    }
...
async fn download_from_buffer(id: String, output: PathBuf, size: Option<u64>, offset: u64, config: &Config) -> Result<()> {
    println!("START");
...
async fn create_texture(input: PathBuf, name: Option<String>, mipmaps: bool, config: &Config) -> Result<()> {
    println!("START");
...
async fn sample_texture(id: String, coordinates: String, output: PathBuf, config: &Config) -> Result<()> {
    println!("START");
...
async fn create_pipeline(shader: PathBuf, name: Option<String>, layout: Option<String>, config: &Config) -> Result<()> {
    println!("START");
...
async fn execute_pipeline(id: String, input: Option<String>, output: Option<PathBuf>, dispatch: Option<String>, config: &Config) -> Result<()> {
    println!("START");
